...
- The minimum system requirements have been updated to CleanroomLoader 0.6.10 Alpha.
- Migrated from Mixinbooter to CleanMix (Mixinbooter 11.x).
...
- Reduced redundant chunk-border lighting work by batching edge checks from all
  queued vertical sections into a single propagation pass
  [#7](https://github.com/Sumire-Labs/Pulsar/pull/7).

## [0.2.3]

### Fixed

- Fixed Celeritas chunk meshes bypassing Pulsar's face-aware neighbouring
  brightness, which caused light to leak through stairs and slabs
  ([#6](https://github.com/Sumire-Labs/Pulsar/issues/6)).
